...
export function registerClassRoutes({ app, storage, requireAuth, requireTeacher }: RouteContext) {
  ...
  // CRUD (teachers only)
  app.post("/api/classes", requireTeacher, async (req: any, res) => {
    try {
  ...
  app.get("/api/classes", requireTeacher, async (req: any, res) => {
    try {
  ...
  app.get("/api/classes/:id", requireTeacher, async (req: any, res) => {
    try {
  ...
  app.put("/api/classes/:id", requireTeacher, async (req: any, res) => {
    try {
      const class_ = await storage.getClassById(req.params.id);
      if (!class_) return res.status(404).json({ message: "Class not found" });
  ...
  app.delete("/api/classes/:id", requireTeacher, async (req: any, res) => {
    try {
      const class_ = await storage.getClassById(req.params.id);
      if (!class_) return res.status(404).json({ message: "Class not found" });
  ...
  // Enrollments
  app.get("/api/classes/:id/enrollments", requireTeacher, async (req: any, res) => {
    try {
      const class_ = await storage.getClassById(req.params.id);
      if (!class_) return res.status(404).json({ message: "Class not found" });
  ...
  app.post("/api/classes/:id/enrollments", requireTeacher, async (req: any, res) => {
    try {
      const class_ = await storage.getClassById(req.params.id);
      if (!class_) return res.status(404).json({ message: "Class not found" });
  ...
  app.delete("/api/classes/:id/enrollments/:userId", requireTeacher, async (req: any, res) => {
    try {
      const class_ = await storage.getClassById(req.params.id);
      if (!class_) return res.status(404).json({ message: "Class not found" });
      if (class_.userId !== req.session.userId!) return res.status(403).json({ message: "Not authorized to manage enrollments" });
  ...
  app.post("/api/classes/:id/students", requireTeacher, async (req: any, res) => {
    try {
  ...
  app.get("/api/users/search", requireTeacher, async (req: any, res) => {
    try {
  ...
  app.get("/api/student/classes", requireAuth, async (req: any, res) => {
    try {
  ...
  // Content assignments
  app.post("/api/content/:contentId/assignments", requireTeacher, async (req: any, res) => {
    try {
  ...
  app.get("/api/content/:contentId/assignments", requireTeacher, async (req: any, res) => {
    try {
      const content = await storage.getContentById(req.params.contentId);
      if (!content) return res.status(404).json({ message: "Content not found" });
  ...
  app.delete("/api/content/:contentId/assignments/:classId", requireTeacher, async (req: any, res) => {
    try {
      const content = await storage.getContentById(req.params.contentId);
      if (!content) return res.status(404).json({ message: "Content not found" });
  ...
  app.get("/api/classes/:id/assignments", requireTeacher, async (req: any, res) => {
    try {
      const class_ = await storage.getClassById(req.params.id);
      if (!class_) return res.status(404).json({ message: "Class not found" });
  ...
  app.get("/api/student/assignments", requireAuth, async (req: any, res) => {
    try {
  ...
  // CSV bulk upload
  app.post("/api/classes/bulk-upload", requireTeacher, async (req: any, res) => {
    try {
