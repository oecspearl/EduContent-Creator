...
export function registerAnalyticsRoutes({ app, storage, requireAuth, requireTeacher }: RouteContext) {
  ...
  app.get("/api/progress/:contentId", requireAuth, async (req: any, res) => {
    try {
  ...
  app.get("/api/quiz-attempts/:contentId", requireAuth, async (req: any, res) => {
    try {
  ...
  // Interaction events
  app.post("/api/interaction-events", requireAuth, async (req: any, res) => {
    try {
  ...
  app.get("/api/interaction-events/:contentId", requireAuth, async (req: any, res) => {
    try {
  ...
  app.get("/api/analytics/content/:contentId", requireAuth, async (req: any, res) => {
    try {
  ...
  app.get("/api/analytics/content/:contentId/learners", requireAuth, async (req: any, res) => {
    try {
      const check = await contentSvc.verifyOwnership(req.params.contentId, req.session.userId!);
      if (!check.ok) return res.status(check.status).json({ message: check.message });
  ...
  app.get("/api/analytics/content/:contentId/questions", requireAuth, async (req: any, res) => {
    try {
      const check = await contentSvc.verifyOwnership(req.params.contentId, req.session.userId!);
      if (!check.ok) return res.status(check.status).json({ message: check.message });
  ...
  app.get("/api/analytics/content/:contentId/performance", requireAuth, async (req: any, res) => {
    try {
      const check = await contentSvc.verifyOwnership(req.params.contentId, req.session.userId!);
      if (!check.ok) return res.status(check.status).json({ message: check.message });
  ...
  app.get("/api/analytics/content/:contentId/score-distribution", requireAuth, async (req: any, res) => {
    try {
      const check = await contentSvc.verifyOwnership(req.params.contentId, req.session.userId!);
      if (!check.ok) return res.status(check.status).json({ message: check.message });
  ...
  // CSV export
  app.get("/api/analytics/content/:contentId/export/csv", requireAuth, async (req: any, res) => {
    try {
      const check = await contentSvc.verifyOwnership(req.params.contentId, req.session.userId!);
      if (!check.ok) return res.status(check.status).json({ message: check.message });
  ...
  app.get("/api/gradebook/:classId", requireTeacher, async (req: any, res) => {
    try {
      ...
      res.status(500).json({ message: "Failed to fetch gradebook data" });
    }
  });

  // CSV export for entire gradebook
  app.get("/api/gradebook/:classId/export/csv", requireTeacher, async (req: any, res) => {
    try {
      const classId = req.params.classId;
      const class_ = await storage.getClassById(classId);
      if (!class_ || class_.userId !== req.session.userId) {
      ...
      console.error("Gradebook CSV export error:", error);
      res.status(500).json({ message: "Failed to export gradebook" });
    }
  });

  // JSON export
  app.get("/api/analytics/content/:contentId/export/json", requireAuth, async (req: any, res) => {
    try {
      const check = await contentSvc.verifyOwnership(req.params.contentId, req.session.userId!);
      if (!check.ok) return res.status(check.status).json({ message: check.message });
  ...
  app.get("/api/analytics/content/:contentId/interactions", requireAuth, async (req: any, res) => {
    try {
      const check = await contentSvc.verifyOwnership(req.params.contentId, req.session.userId!);
      if (!check.ok) return res.status(check.status).json({ message: check.message });
      ...
      console.error("Get interaction analytics error:", error);
      res.status(500).json({ message: "Failed to fetch interaction analytics" });
    }
  });

  // AI-generated progress report for a student in a class
  app.post("/api/gradebook/:classId/report/:studentId", requireTeacher, aiGenerationRateLimit, async (req: any, res) => {
    try {
